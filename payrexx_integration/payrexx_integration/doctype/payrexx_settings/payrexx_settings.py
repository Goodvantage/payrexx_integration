...
class PayrexxSettings(Document):
	...
	def create_static_qr(self, webshop_url: str) -> dict:
		...
		Not whitelisted — callers own permission checks.
		"""
		...
		try:
			...
			frappe.log_error(frappe.get_traceback(), "Payrexx create_static_qr")
	...
	def delete_static_qr(self, qr_code_uuid: str) -> None:
		...
		dashboard cannot wedge local cleanup. Not whitelisted — callers own
		permission checks.
		"""
		...
		try:
			...
			if get_http_status(exc) == 404:
				return
			frappe.log_error(frappe.get_traceback(), "Payrexx delete_static_qr")
